...
function Field({
  ...
  const checkWin = () => {
    const checked = document.querySelectorAll(`.${s.cell}`);
    let bombCount = 0;
    let checkCount = 0;

    for (let i = 0; i < checked.length; i++) {
      const row = Math.floor(checked[i].id / (m + 2));
      const col = checked[i].id % (m + 2) - 1;
      ...
      const zeroesCollect = (a, fieldAns, row, col) => {
        ...
        for (let k = 0; k < dx.length; k++) {
          if (a.current.children[row + dx[k]].children[col + dy[k]].getAttribute('checked') == null
                       && fieldAns[row + dx[k]][col + dy[k]] === 0 && row + dx[k] > 0
                       && row + dx[k] < n + 1 && col + dy[k] > 0 && col + dy[k] < m + 1) {
    ...
    <div className={s.field} ref={a}>
      {
           field.map((rows, row) => (
             <div className={s.row}>
               {rows.map((item, col) => <div className={s.cell} id={(row) * (m + 2) + col + 1} onClick={() => checkCell(row, col)} onContextMenu={onHandleRightClick(row, col)} />)}
             </div>
           ))
}
    </div>
